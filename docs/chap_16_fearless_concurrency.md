...
Calling `join` on the handle blocks the thread currently running (main) until the thread represented by the handle terminates. **Blocking a thread means that thread is prevented from performing work or exiting.** Because we’ve put the call to `join` after the main thread’s for loop, running Listing 16-2 should produce output similar to this:
...
```
The main thread will wait for the spawned thread to finish and then run its for loop, so the output won’t be interleaved anymore (not even concurrent anymore!), as shown here:
