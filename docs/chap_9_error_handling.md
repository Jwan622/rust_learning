...
If this function succeeds without any problems, the code that calls this function will receive an `Ok` value that holds a `String` — the username that this function `read_to_string` read from the file and put into the variable `username`. If this function encounters any problems, the calling code will receive an Err value that holds an instance of io::Error that contains more information about what the problems were. We chose io::Error as the return type of this function because that happens to be the type of the error value returned from both of the operations we’re calling in this function’s body that might fail: the File::open function and the read_to_string method.
...
### The ? Operator, shortcut for propogating errors.
...
### What's the difference between ? and using match?
...
fn read_username_from_file() -> Result<String, io::Error> {
    ...
    File::open("hello.txt")?.read_to_string(&mut username)?; // you can chain ? propogation. wow.
...
Listing 9-9 shows a way to make this even shorter using `fs::read_to_string.`. Even shorter! We return a String that contains the contents of `hello.txt`, same as before.
...
### When we know more than the compiler

Using `expect` or `unwrap` is fine and documents that this should never error. You might want to use it in tests too.
...
Why is this allowed by the compiler? Since the `expect` method is guaranteed to return an IpAddr when successful, it matches the type `home: IpAddr`.
...
The compiler will produce an error if you do not handle the Result returned by the parse method. In Rust, the parse method returns a Result type because parsing can fail. This means you need to handle both the Ok and Err variants of the Result to ensure your code is robust and safe.

When you call the parse method without handling the result, the compiler will complain because you are ignoring a possible error. Rust requires you to explicitly handle errors to ensure that they are not silently ignored.

```rust
use std::net::IpAddr;

fn main() {
    let home: IpAddr = "127.0.0.1".parse(); // This will cause a compile-time error
}
```
this error:
...
error[E0308]: mismatched types
 --> src/main.rs:4:22
  |
4 |     let home: IpAddr = "127.0.0.1".parse();
  |                      ^^^^^^^^^^^^^^^^^^^^ expected `IpAddr`, found enum `Result`
  |
  = note: expected type `IpAddr`
             found type `Result<IpAddr, _>`
```
...
When you expect a failure, or can handle it, return an error Result. However, when failure is expected, it’s more appropriate to return a Result than to make a panic! call. Examples include a parser being given malformed data or an HTTP request returning a status that indicates you have hit a rate limit. In these cases, returning a Result indicates that failure is an expected possibility that the calling code must decide how to handle.
...
Instead, we can make a new type (A struct) and put the validations in a function to create an instance of the type rather than repeating the validations everywhere. That way, it’s safe for other functions to **use the new type (the struct) in their signatures and confidently use the values they receive**. Listing 9-13 shows one way to define a `Guess` type that will only create an instance of `Guess` if the new function receives a value between 1 and 100.
...
// Listing 9-13: A Guess type that will only continue with values between 1 and 100. this Guess can be used in functions, fully knowing that the value is between 1 and 100 now, removing the need for checks everywhere.
```
